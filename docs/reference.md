...
Validation code is not generated by `nanopb_generator.py`. It is produced by a
separate protoc plugin, `protoc-gen-nanopb-validate`, which writes
`*_validate.h` / `*_validate.c`. Given `--filter=<fully.qualified.Message>` it
additionally injects a packet filter --
`<Message>_filter_udp()` / `<Message>_filter_tcp()` -- into the generated
`.pb.h` / `.pb.c`. See :doc:`validation` for how to invoke it.
